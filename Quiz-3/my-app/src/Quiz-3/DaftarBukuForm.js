...
const DaftarBukuForm = () =>{
  ...
  const [input, setInput] = useState({title: "", description: "", review: "",release_year:"",totalPage:0,price:"",image_url:""})
  useEffect(()=>{
    if (daftarBuku.statusForm === "changeToEdit"){
      ...
      .then(res => {
        ...
        setInput({title: dataBuku.title, description: dataBuku.description, review: dataBuku.review,release_year:dataBuku.release_year,totalPage:dataBuku.totalPage,price:dataBuku.price,image_url:dataBuku.image_url})
        setDaftarBuku({...daftarBuku, statusForm: "edit"})
      })
  ...
  const handleSubmit = (event) =>{
    ...
    let review = input.review.toString()
    let release_year = input.release_year.toString()
    let totalPage = input.totalPage
    ...
    if (daftarBuku.statusForm === "create"){        
      ...
      axios.put(`http://backendexample.sanbercloud.com/api/fruits/${daftarBuku.selectedId}`, {name, price, weight: input.weight})
      .then(() => {
      ...
      <div style={{width: "50%", margin: "0 auto", display: "block"}}>
        <div style={{border: "1px solid #aaa", padding: "20px"}}>
          <form onSubmit={handleSubmit}>
            ...
            <input style={{float: "right"}} type="string" name="image_url" value={input.image_url} onChange={handleChange}/>
            <br/>
            <br/>
